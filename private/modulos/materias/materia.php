        ...
        if ($this->respuesta['msg'] == 'ok') {
            if ($accion == 'nuevo') {
                return $this->db->consultasql(
                    'INSERT INTO materias (codigo, nombre, uv, codigo_transaccion) VALUES (?, ?, ?, ?)', 
                    $this->datos['codigo'], $this->datos['nombre'], $this->datos['uv'], $this->datos['codigo_transaccion']
                );
            } else if ($accion == 'modificar') {
                return $this->db->consultasql(
                    'UPDATE materias SET codigo=?, nombre=?, uv=? WHERE codigo_transaccion = ?', 
                    $this->datos['codigo'], $this->datos['nombre'], $this->datos['uv'], $this->datos['codigo_transaccion']
                );
            } else if ($accion == 'eliminar') {
                return $this->db->consultasql(
                    'DELETE FROM materias WHERE codigo_transaccion = ?', 
